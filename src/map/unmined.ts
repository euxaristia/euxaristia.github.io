...
interface UnminedInternalOptions {
    worldName: string;
    minZoom: number;
    maxZoom: number;
    defaultZoom?: number;
...
class RegionMap {
    ...
    hasTile(tileX: number, tileZ: number, unminedZoomLevel: number): boolean {
        ...
        const tileBlockPoint: Point = {
            x: tileX * tileBlockSize,
            z: tileZ * tileBlockSize
        };

        const tileRegionPoint: Point = {
...
class RedDotMarker {
    ...
    constructor(map: any, dataProjection: any, viewProjection: any) {
        this.#map = map;
        ...
        window.addEventListener('hashchange', (e: HashChangeEvent) => { this.#hashChanged(e.newURL); });
    ...
    static getCoordinatesFromUrlHash(hash: string): [number, number] | undefined {
    ...
    static getUrlHashWithCoordinates(hash: string | null, coordinates: [number, number] | undefined): string {
        const base = hash ?? '#';
...
class Unmined {
    olMap: any = null;
    gridLayer: any = null;
    coordinateLayer: any = null;
    viewProjection: any = null;
    dataProjection: any = null;
    regionMap: RegionMap | null = null;
    markersLayer: any = null;
    playerMarkersLayer: any = null;
    redDotMarker!: RedDotMarker;

    #scaleLine: any = null;
    #options!: UnminedInternalOptions;

    static defaultOptions: Partial<UnminedInternalOptions> = {
        enableGrid: true,
        showGrid: true,
        binaryGrid: true,
        showScaleBar: false,
    ...
    constructor(mapElement: HTMLElement, options: MapProperties, regions: RegionEntry[]) {
        const worldTileSize = 256;

        this.#options = { ...Unmined.defaultOptions, ...options } as UnminedInternalOptions;
        ...
        const resolutions = new Array<number>(mapZoomLevels + 1);
        ...
        const unminedLayer = new ol.layer.Tile({
            source: new ol.source.XYZ({
                ...
                tileSize: worldTileSize / dpiScale,
                tileUrlFunction: (coordinate: [number, number, number]) => {
                    ...
                    if (this.regionMap!.hasTile(tileX, tileY, worldZoom)) {
                        return ('tiles/zoom.{z}/{xd}/{yd}/tile.{x}.{y}.' + this.#options.imageFormat)
    ...
    placeRedDotMarker(coordinates: [number, number] | undefined): void {
        this.redDotMarker.setCoordinates(coordinates);
    }

    createMarkersLayer(markers: MarkerItem[]): any {
        const features: any[] = [];

        for (const item of markers) {
            const feature = new ol.Feature({
                geometry: new ol.geom.Point(
                    ol.proj.transform([item.x, item.z], this.dataProjection, this.viewProjection)
                )
            });
    ...
    static playerToMarker(player: PlayerData): MarkerItem {
        return {
    ...
    static createPlayerMarkers(players: PlayerData[]): MarkerItem[] {
    ...
    #createGraticuleLayer(coord: boolean): any {
        const intervalCount: number = this.olMap.getView().getMaxZoom() + 2;
        const graticuleIntervals = new Array<number>(intervalCount);

        if (this.#options.binaryGrid) {
            let base = 16;
            for (let z = 0; z < intervalCount; ++z) {
                const intervalInDegrees = ol.proj.transform(
                    [base, base], this.dataProjection, this.viewProjection
                )[0];
            ...
            for (let z = 0; z < intervalCount; ++z) {
                ...
                const intervalInDegrees = ol.proj.transform(
                    [intervalInBlocks, intervalInBlocks], this.dataProjection, this.viewProjection
                )[0];
                graticuleIntervals[intervalCount - 1 - z] = intervalInDegrees;
        ...
        return new ol.layer.Graticule({
            ...
            lonLabelFormatter: coord ? (lon: number) => {
                const c = new ol.geom.Point(
                    ol.proj.transform([lon, 0], this.viewProjection, this.dataProjection)
                ).getFirstCoordinate();
                const l = Math.round(c[0]);
                return l === 0 ? "x = 0" : l.toString();
            } : undefined,
            latLabelFormatter: coord ? (lat: number) => {
                const c = new ol.geom.Point(
                    ol.proj.transform([0, lat], this.viewProjection, this.dataProjection)
                ).getFirstCoordinate();
    ...
    createContextMenu(): any {
        ...
        contextmenu.on('open', (evt: any) => {
            const coordinates: [number, number] = ol.proj.transform(
    ...
    loadSettings(): void {
        let mapSettings: SavedSettings | undefined;
        try {
            const s = localStorage.getItem("unminedSettings");
            if (s) mapSettings = JSON.parse(s) as SavedSettings;
        } catch {
    ...
    saveSettings(): void {
        const mapSettings: SavedSettings = {
    ...
    updateScaleBar(): void {
        if (!this.#options.showScaleBar && this.#scaleLine) {
            ...
            this.#scaleLine = new ol.control.ScaleLine({ bar: true, minWidth: 200 });
            this.olMap.addControl(this.#scaleLine);
        }
    }

    #initProjections(maxCoordValue: number): void {
        ...
        ol.proj.addCoordinateTransforms(
            this.viewProjection,
            this.dataProjection,
            (coordinate: number[]) => [coordinate[0] * blocksPerDegrees, -coordinate[1] * blocksPerDegrees],
            (coordinate: number[]) => [coordinate[0] / blocksPerDegrees, -coordinate[1] / blocksPerDegrees]
        );
    }
}
